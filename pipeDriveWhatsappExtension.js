...
class pipeDriveWhatsappExtension {
    ...
    destroy() {
        if (document.querySelector('.whatsappPipeline')) document.querySelector('.whatsappPipeline').remove();
    }
    ...
    phoneSanitize(phone) {
        ...
        return cleanNumber.indexOf('+55') === 0 ? cleanNumber.replace('+', '') : `55${cleanNumber}`;
    }
    ...
    phonesSetEvents() {
        ...
        this.getPhones().forEach(phone => {
            ...
            cell.removeEventListener('mouseenter', () => this.phoneMouseEvent(cell, phone));
            cell.addEventListener('mouseenter', () => this.phoneMouseEvent(cell, phone));
        });
        this.queue.shift();
    }
    ...
    createButton() {
        ...
        this.whtpipeButtonElement.addEventListener('click', () => {
            ...
            if (this.whatsappMessage) {
                ...
                text = this.whatsappMessage.replace('{usuario}', user);
            }
            window.open(`//api.whatsapp.com/send?phone=${this.currentNumber}&text=${text}`);
        });
    }
}
