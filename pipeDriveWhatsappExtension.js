...
class pipeDriveWhatsappExtension {
  ...
  init() {
    ...
    this.createButton();
    if (window.location.href.indexOf("deal") > 0) this.phoneEvent();
  }
  ...
  phoneEvent() {
    let timer = setInterval(() => {
      const phone = document.querySelector(
        ".phoneField .salesPhone .phoneNumber .value"
      );
      if (phone) {
        let { x, y } = phone.getBoundingClientRect();
        this.whtpipeButtonElement.style.display = "block";
        this.whtpipeButtonElement.style.left = `${Math.round(x - 84)}px`;
        this.whtpipeButtonElement.style.top = `${Math.round(y - 4)}px`;
        this.currentNumber = this.phoneSanitize(phone);
        clearInterval(timer);
      }
    }, 2000);
